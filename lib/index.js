...
exports.register = (server, options) => {
    ...
    // Establish cache
    server.expose('cache', new NodeCache(internals.settings.cache));

    // Establish Good reporter stream
    server.expose('GoodTracerStream', internals.goodTracerStreamFactory(server, internals.settings));

    const {
        traceUUIDHeader, traceSeqIDHeader, enableStatsRoute, baseRoute
    } = internals.settings;
    ...
    server.ext('onRequest', (request, h) => {
        ...
        const cache = get(request, ['server', 'plugins', NAME, 'cache']);
        cache.set(requestId, {
            uuid: tracerUUID,
