...
exports.register = (server, options) => {
    ...
    server.ext('onPreResponse', (request, h) => {
        set(request, ['response', 'headers', traceUUIDHeader], get(request, ['plugins', NAME, traceUUIDHeader]));
        set(request, ['response', 'headers', traceSeqIDHeader], get(request, ['plugins', NAME, traceSeqIDHeader]));

        debug('Response Trace headers: %o', {
            [traceUUIDHeader]: get(request, ['response', 'headers', traceUUIDHeader]),
            [traceSeqIDHeader]: get(request, ['response', 'headers', traceSeqIDHeader])
        });
